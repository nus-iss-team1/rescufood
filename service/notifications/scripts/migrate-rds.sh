...
# This service's own database, distinct from profile's - see
# infrastructure/README.md's Notification service section.
...
aws ssm start-session --region "$REGION" \
  ...
  >/tmp/notifications-migrate-rds-tunnel.log 2>&1 &
TUNNEL_PID=$!
...
echo "==> granting the notifications role privileges on migrated tables"
DATABASE_URL="$DB_URL" NODE_PATH="${SCRIPT_DIR}/../node_modules" node -e '
const { Client } = require("pg");
(async () => {
  const client = new Client({ connectionString: process.env.DATABASE_URL });
  await client.connect();
  await client.query("GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO notifications");
  await client.query("GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO notifications");
  await client.end();
})();
'
